	...
	port.on('data',function(data){
		if(data=='$'){
			...
			if(buf.includes("$GPGGA")){
				...
				let gps={};
				str=buf.split(',');
				gps.lat=parseFloat(str[2]);
				gps.lng=parseFloat(str[4]);
				gps.lat = parseInt(gps.lat/100)+parseInt(gps.lat-parseInt(gps.lat/100)*100)/60+(gps.lat-parseInt(gps.lat))/60;
				gps.lng = parseInt(gps.lng/100)+parseInt(gps.lng-parseInt(gps.lng/100)*100)/60+(gps.lng-parseInt(gps.lng))/60;
				obj.gps=gps;
			}
			buf="";
		}
